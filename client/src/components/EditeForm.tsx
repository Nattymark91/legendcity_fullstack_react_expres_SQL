...
import { editeMail } from '../store/action-creator/mailsAction';
...
const EditeForm: React.FC = ({mail}) => {
    const {currentGift} = useTypedSelector(state => state.gift);
    const [giftsVisible, setGiftsVisible] = useState(false);
    const {fetchGifts, fetchOneGifts } = useActions();
    const [currGift, setCurrGift] = useState(mail.giftId);
    const [giftQuantity, setGiftQuantity] = useState(currentGift.quantity);
    ...
    const handleClose = () => setOpen(false);
      ...
      useEffect(() => {
        fetchGifts(), fetchOneGifts(mail.giftId)
    }, [])
    ...
    return (
        ...
        onSubmit={(values) => {
            editeMail(mail.id, values).then(() => window.location.reload())
        }}>
           ...
           <div style={{color: 'red'}}>{errors.name}</div>
            ...
            <div>
            {currGift ? <p>Выбран подарок № {currGift}</p> : <p>Подарок не выбран</p>}
            {errors.giftId ? (
           ...
           <Button variant="contained" onClick={handleOpen} color="primary">Изменить</Button>
           <Button variant="contained" onClick={()=>console.log(giftQuantity)} color="primary">Показать</Button>
           <Modal
              open={open}
              onClose={handleClose}
            ...
            >
                  ...
                  <Button type="submit" form="edite">Да</Button>
                  <Button variant="contained" onClick={handleClose} color="secondary">Нет</Button>
              </div>
            </Modal>
            <GiftsModal open={giftsVisible} onClose={() => {setGiftsVisible(false); 
                  setFieldValue("giftId", currentGift.id); 
                  setCurrGift(currentGift.id);
                  setGiftQuantity(currentGift.quantity);
                }
              }
             /> 
